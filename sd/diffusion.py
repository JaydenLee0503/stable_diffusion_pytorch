...
class UNET_ResidualBlock(nn.Module):
    ...
    def forward(self, feature, time):
        # fearure: (Batch_Size, In_Channels, Height, Width)
        # time (1, 1280)
...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_head: int, n_embd: int, d_context=768):
        ...
        self.linear_geglu_2 = nn.Linear(4 * channels, 4 * channels)

        self.conv_output = nn.Conv2d(channels, channels, kernel_size=1, padding=0)

    def forward(self, x: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        # x: (Batch_Size, Channels, Height, Width)
        # context: (Batch_Size, Sequence_Length, Channels)
...
class UNET(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoders = nn.Module([
            ...
            # (Batch_Size, 4, Height / 16, Width / 16) -> (Batch_Size, 4, Height / 16, Width / 16)
...
class UNET_OutputLayer(nn.Module):
    def __int__(self, in_channels: int, out_channels: int):
        super().__init__()
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        output = self.unet(latent, time, context)
